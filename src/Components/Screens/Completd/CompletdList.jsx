...
const CompletdList = ({ data, updateStatus, deleteTask }) => {
  const [modal, setModal] = useState(false);
  const newData = data.filter((item) => item.status === "Completed");
              ...
              {newData.map((item, index) => (
                <View
                  ...
                  <View className="flex-row justify-between items-center mt-2">
                    <View className="bg-[#82D616] p-1 md:w-[40px] md:h-[20px] sm:w-[30px] sm:h-[16px] rounded flex justify-center items-center">
